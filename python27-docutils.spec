...
for file in %{buildroot}/%{_bindir}/*.py; do
    j=`basename $file`
    jj=`basename $file .py`2.7
    jk=`basename $file .py`
    mv $file `dirname $file`/$jj
    ln -s %{_bindir}/$jj %{buildroot}%{pybindir}/$j
    ln -s %{_bindir}/$jj %{buildroot}%{pybindir}/$jk
done
...
%changelog
* Thu Sep 18 2014 Brandon Pierce <brandon@ihashacks.com - 0.8.1-3
