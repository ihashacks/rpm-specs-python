...
%global pybindir /usr/lib/python%{pybasever}/bin
...
rm -f %{buildroot}%{_bindir}/python-config %{buildroot}%{_bindir}/%{python}-config
rm -f %{buildroot}%{_bindir}/python2
rm -f %{buildroot}%{_bindir}/python2-config
ln -sf %{_bindir}/python%{pybasever}-config %{buildroot}%{_bindir}/%{python}-config
ln -sf %{_bindir}/python%{pybasever}-config %{buildroot}%{pybindir}
ln -sf %{_bindir}/python%{pybasever}-config %{buildroot}%{pybindir}/python2-config
ln -sf %{_bindir}/python%{pybasever}-config %{buildroot}%{pybindir}/python27-config
ln -sf %{_bindir}/python%{pybasever}-config %{buildroot}%{pybindir}/python-config
%if 0%{?with_debug_build}
rm -f %{buildroot}%{_bindir}/python-debug
mv %{buildroot}%{_bindir}/python-debug-config %{buildroot}%{_bindir}/%{python}-debug-config
rm -f %{buildroot}%{_bindir}/python2-debug
rm -f %{buildroot}%{_bindir}/python2-debug-config
ln -sf %{_bindir}/python%{pybasever}-debug-config %{buildroot}%{pybindir}
ln -sf %{_bindir}/python%{pybasever}-debug-config %{buildroot}%{pybindir}/python-debug
ln -sf %{_bindir}/python%{pybasever}-debug-config %{buildroot}%{pybindir}/python2-debug
ln -sf %{_bindir}/python%{pybasever}-debug-config %{buildroot}%{pybindir}/python27-debug
%endif # with_debug_build
%endif
...
mv 2to3 2to3%{__python_ver}
ln -s /usr/bin/2to3%{__python_ver} %{buildroot}%{pybindir}/2to3
mv idle idle%{__python_ver}
ln -s /usr/bin/idle%{__python_ver} %{buildroot}%{pybindir}/idle
mv pynche pynche%{__python_ver}
ln -s /usr/bin/pynche%{__python_ver} %{buildroot}%{pybindir}/pynche
mv pygettext.py pygettext%{__python_ver}.py
ln -s /usr/bin/pygettext%{__python_ver}.py %{buildroot}%{pybindir}/pygettext.py
mv msgfmt.py msgfmt%{__python_ver}.py
ln -s /usr/bin/msgfmt%{__python_ver}.py %{buildroot}%{pybindir}/msgfmt.py
mv smtpd.py smtpd%{__python_ver}.py
ln -s /usr/bin/smtpd%{__python_ver}.py %{buildroot}%{pybindir}/smtpd.py
mv pydoc pydoc%{__python_ver}
ln -s /usr/bin/pydoc%{__python_ver} %{buildroot}%{pybindir}/pydoc
popd
%endif
...
ln -s %{_bindir}/python%{pybasever} %{buildroot}%{pybindir}/python
ln -s %{_bindir}/python%{pybasever} %{buildroot}%{pybindir}/python2
mkdir -p %{buildroot}/usr/share/Modules/modulefiles/python
cat > %{buildroot}/usr/share/Modules/modulefiles/python/2.7 <<ENDPY
#%Module

# NOTE: This is an automatically-generated file!  (generated by the
# python27 RPM).  Any changes made here will be lost a) if the RPM is
# uninstalled, or b) if the RPM is upgraded or uninstalled.

proc ModulesHelp { } {
   puts stderr "This module adds python 2.7 to relevant paths"
}

module-whatis   "Sets up Python 2.7 as default in your environment"


prepend-path PATH "%{pybindir}"
ENDPY
...
%{pybindir}/python2
%{pybindir}/pydoc
/usr/share/Modules/modulefiles/python/2.7
...
%else
%{_bindir}/python27-config
%endif
%{_bindir}/python%{pybasever}-config
%{pybindir}/python-config
%{pybindir}/python2-config
%{pybindir}/python27-config
%{pybindir}/python2.7-config
...
%changelog
* Thu Sep 18 2014 Brandon Pierce <brandon@ihashacks.com - 2.7.3-6.1
- Rebuilt for CentOS 6

* Wed Jul 11 2012 Benjamin Rose <benrose@cs.princeton.edu> - 2.7.3-6.1
- Port to run as python27 on PUIAS, which uses python 2.6 as its core.
