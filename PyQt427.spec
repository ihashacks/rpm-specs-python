...
BuildRequires: dbus-devel dbus-python27-devel
...
BuildRequires: sip27-devel

%global python_sitelib  %(%{__python} -c "from distutils.sysconfig import get_python_lib; print get_python_lib()")
%global python_sitearch %(%{__python} -c "from distutils.sysconfig import get_python_lib; print get_python_lib(1)")

BuildRequires: sip27-devel >= 4.9.3

Requires: dbus-python27
...
%{__python} configure.py \
  --confirm-license \
  --qmake=%{_qt4_qmake} \
  --dbus=%{_includedir}/python2.7/dbus-1.0 \
  --verbose 
...
mv %{buildroot}%{python_sitearch}/PyQt4/uic/pyuic.py \
   %{buildroot}%{_bindir}/pyuic427
ln -s %{_bindir}/pyuic427 \
      %{buildroot}%{python_sitearch}/PyQt4/uic/pyuic.py
ln -s %{_bindir}/pyuic427 %{buildroot}%{pybindir}/pyuic4
mv %{buildroot}%{_bindir}/pylupdate4 %{buildroot}%{_bindir}/pylupdate427
ln -s %{_bindir}/pylupdate427 %{buildroot}%{pybindir}/pylupdate4
mv %{buildroot}%{_bindir}/pyrcc4 %{buildroot}%{_bindir}/pyrcc427
ln -s %{_bindir}/pyrcc427 %{buildroot}%{pybindir}/pyrcc4

mkdir -p %{buildroot}%{_qt4_plugindir}/designer/python27

%check
# verify phonon built ok
test -f %{buildroot}%{python_sitearch}/PyQt4/phonon.so
test -d %{buildroot}%{_datadir}/sip27/PyQt4/phonon
...
%changelog
* Thu Sep 18 2014 Brandon Pierce <brandon@ihashacks.com - 4.6.2-9
