...
Provides:       wxPythonGTK27 = %{version}-%{release}
...
cd wxPython
%{__python} setup.py %{buildflags} install --root=$RPM_BUILD_ROOT
...
for i in *; do
	mv $i ${i}27
	ln -s %{_bindir}/${i}27 $RPM_BUILD_ROOT%{pybindir}/$i
done
popd
mv $RPM_BUILD_ROOT%{_includedir}/wx-2.8 $RPM_BUILD_ROOT%{_includedir}/wx27-2.8
...
%changelog
* Thu Sep 18 2014 Brandon Pierce <brandon@ihashacks.com - 2.8.12.0-1
